...
func (s msgServer) CreateProject(goCtx context.Context, msg *types.MsgCreateProject) (*types.MsgCreateProjectResponse, error) {
	...
	if _, err = createAccountInProjectAccounts(ctx, k, msg.ProjectDid, FuryAccountFeesId); err != nil {
		return nil, err
	}
	if _, err = createAccountInProjectAccounts(ctx, k, msg.ProjectDid, FuryAccountPayFeesId); err != nil {
		return nil, err
	}
...
func payoutFees(ctx sdk.Context, k Keeper, bk bankkeeper.Keeper, projectDid didexported.Did) error {
	...
	_, err := payAllFeesToAddress(ctx, k, bk, projectDid, FuryAccountPayFeesId, FuryAccountFeesId)
	if err != nil {
		return err
	}

	_, err = payAllFeesToAddress(ctx, k, bk, projectDid, InitiatingNodeAccountPayFeesId, FuryAccountFeesId)
	if err != nil {
		return err
	}

	furyDid := k.GetParams(ctx).FuryDid
	amount := getFuryAmount(ctx, k, bk, projectDid, FuryAccountFeesId)
	err = payoutAndRecon(ctx, k, bk, projectDid, FuryAccountFeesId, iidtypes.DIDFragment(furyDid), amount)
	if err != nil {
		return err
	}
...
func payAllFeesToAddress(ctx sdk.Context, k Keeper, bk bankkeeper.Keeper, projectDid didexported.Did,
	...
	feesToPay := getFuryAmount(ctx, k, bk, projectDid, sendingAddress)
...
func getFuryAmount(ctx sdk.Context, k Keeper, bk bankkeeper.Keeper, projectDid didexported.Did, accountID types.InternalAccountID) sdk.Coin {
	...
	if found {
		...
		return sdk.NewCoin(furytypes.FuryNativeToken, coins.AmountOf(furytypes.FuryNativeToken))
	}
	return sdk.NewCoin(furytypes.FuryNativeToken, sdk.ZeroInt())
}
...
func (s msgServer) CreateEvaluation(goCtx context.Context, msg *types.MsgCreateEvaluation) (*types.MsgCreateEvaluationResponse, error) {
	...
	if err == nil {
		// Get fury address
		furyAddr, err := getAccountInProjectAccounts(ctx, k, msg.ProjectDid,
			FuryAccountPayFeesId)
		if err != nil {
			return nil, err
		}
...
func (s msgServer) WithdrawFunds(goCtx context.Context, msg *types.MsgWithdrawFunds) (*types.MsgWithdrawFundsResponse, error) {
	...
	amountCoin := sdk.NewCoin(furytypes.FuryNativeToken, amount)
...
func payoutAndRecon(ctx sdk.Context, k Keeper, bk bankkeeper.Keeper, projectDid didexported.Did,
	...
	furyBalance := getFuryAmount(ctx, k, bk, projectDid, fromAccountId)
	if furyBalance.IsLT(amount) {
