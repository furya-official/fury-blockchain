...
import (
	...
	entitycontracts "github.com/furya-official/fury-blockchain/x/entity/types/contracts"
...
func (k Keeper) CreateEntity(ctx sdk.Context, msg *types.MsgCreateEntity) (types.MsgCreateEntityResponse, error) {
	...
	entityId := fmt.Sprintf("did:fury:entity:%s:%s", msg.EntityType, base58.Encode(pubKey.Bytes()[:16]))
	...
	_, err = k.WasmKeeper.Execute(ctx, nftContractAddress, address, finalMessage, sdk.NewCoins(sdk.NewCoin("ufury", sdk.ZeroInt())))
	if err != nil {
		return types.MsgCreateEntityResponse{}, err
	}
...
func (k Keeper) TransferEntity(ctx sdk.Context, msg *types.MsgTransferEntity) (*types.MsgTransferEntityResponse, error) {
	...
	_, err = k.WasmKeeper.Execute(ctx, nftContractAddress, controllerAddress, finalMessage, sdk.NewCoins(sdk.NewCoin("ufury", sdk.ZeroInt())))
	if err != nil {
		return nil, err
	}
