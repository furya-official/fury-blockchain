...
import (
	...
	"github.com/furya-official/fury-blockchain/x/token/types"
	"github.com/furya-official/fury-blockchain/x/token/types/contracts/cw20"
	"github.com/furya-official/fury-blockchain/x/token/types/contracts/cw721"
	"github.com/furya-official/fury-blockchain/x/token/types/contracts/fury1155"
)
...
func (s msgServer) SetupMinter(goCtx context.Context, msg *types.MsgSetupMinter) (*types.MsgSetupMinterResponse, error) {
	...
	case *types.MsgSetupMinter_Cw1155:
		codeId = params.GetFury1155ContractCode()
		label = fmt.Sprintf("%s-cw1155-contract", msg.MinterDid.String())
		contractType = types.ContractType_FURY1155
		encodedInitiateMessage, err = fury1155.InstantiateMsg{
			Minter: minterAddress.String(),
		}.Marshal()
	default:
...
func (s msgServer) MintToken(goCtx context.Context, msg *types.MsgMint) (*types.MsgMintResponse, error) {
	...
	case *types.MsgMint_Cw1155:
		if tokenMinter.ContractType != types.ContractType_FURY1155 {
			return &types.MsgMintResponse{}, sdkerrors.ErrInvalidType.Wrap("selected contract is not a cw1155 contract type")
		}
		encodedMintMessage, err = fury1155.Mint{
			To:      ownerAddress.String(),
			TokenId: fury1155.TokenId(msg.OwnerDid.Did()),
