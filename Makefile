...
ldflags = \
    ...
    -X "github.com/furya-official/fury-blockchain/version.BuildTags=$(build_tags_comma_sep)" \
...
build: go.sum
ifeq ($(OS),Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o build/furyd.exe ./cmd/furyd
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/furyd ./cmd/furyd
endif
...
draw-deps:
	...
	@goviz -i github.com/furya-official/fury-blockchain/cmd/furyd -d 2 | dot -Tpng -o dependency-graph.png
