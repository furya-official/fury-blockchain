...
import (
	...
	"github.com/furya-official/fury-blockchain/compatibility"
...
var (
	...
	expectedMinGasPrices       = "0.025" + furytypes.FuryNativeToken
)
...
func SignDataRequest(clientCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// all messages must be of type fury.FuryMsg
		furyMsg, ok := msg.(furytypes.FuryMsg)
		if !ok {
			rest.WriteErrorResponse(w, http.StatusBadRequest, "msg must be fury.FuryMsg")
			return
		}
		...
		case projecttypes.TypeMsgCreateProject:
			...
			stdSignMsg = furyMsg.(*projecttypes.MsgCreateProject).ToStdSignMsg(
		...
		default:
			...
			fees := sdk.NewCoins(sdk.NewCoin(furytypes.FuryNativeToken, sdk.OneInt()))
