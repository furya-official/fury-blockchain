...
FEE1=$(yes $PASSWORD | furyd keys show fee -a)
FEE2=$(yes $PASSWORD | furyd keys show fee2 -a)
FEE3=$(yes $PASSWORD | furyd keys show fee3 -a)
FEE4=$(yes $PASSWORD | furyd keys show fee4 -a)
RESERVE_OUT=$(yes $PASSWORD | furyd keys show reserveOut -a)

BOND1_DID="did:fury:U7GK8p8rVhJMKhBVRCJJ8c"
BOND2_DID="did:fury:JHcN95bkS4aAWk3TKXapA2"
BOND3_DID="did:fury:48PVm1uyF6QVDSPdGRWw4T"
BOND4_DID="did:fury:RYLHkfNpbA8Losy68jt4yF"
#BOND1_DID_FULL='{
...
#}'
#BOND2_DID_FULL='{
#  "did":"did:fury:JHcN95bkS4aAWk3TKXapA2",
...
#}'
#BOND3_DID_FULL='{
#  "did":"did:fury:48PVm1uyF6QVDSPdGRWw4T",
...
#}'
#BOND4_DID_FULL='{
#  "did":"did:fury:RYLHkfNpbA8Losy68jt4yF",
...
PROJECT1_DID="did:fury:U7GK8p8rVhJMKhBVRCJJ8c"
PROJECT2_DID="did:fury:JHcN95bkS4aAWk3TKXapA2"
PROJECT1_DID_FULL='{
  "did":"did:fury:U7GK8p8rVhJMKhBVRCJJ8c",
  "verifyKey":"FmwNAfvV2xEqHwszrVJVBR3JgQ8AFCQEVzo1p6x4L8VW",
  "encryptionPublicKey":"domKpTpjrHQtKUnaFLjCuDLe2oHeS4b1sKt7yU9cq7m",
...
SHAUN_DID="did:fury:U4tSpzzv91HHqWW1YmFkHJ"
MIGUEL_DID_FULL='{
...
PAYMENT_RECIPIENTS='[
  {
    "address": "fury107pmtx9wyndup8f9lgj6d7dnfq5kuf3sapg0vx",
    "percentage": "100"
  }
]'
...
furyd_tx did add-did-doc "$MIGUEL_DID_FULL"
echo "Ledgering DID 2/3..."
furyd_tx did add-did-doc "$FRANCESCO_DID_FULL"
echo "Ledgering DID 3/3..."
furyd_tx did add-did-doc "$SHAUN_DID_FULL" --broadcast-mode block

# Adding KYC credentials
echo "Adding KYC credential 1/1..."
furyd_tx did add-kyc-credential "$MIGUEL_DID" "$FRANCESCO_DID_FULL" --broadcast-mode block
...
furyd_tx bonds buy 5token1 "100000res" "$BOND1_DID" "$MIGUEL_DID_FULL" --broadcast-mode block
echo "Buying 5token2 from Miguel..."
furyd_tx bonds buy 5token2 "100000res" "$BOND2_DID" "$MIGUEL_DID_FULL" --broadcast-mode block

# Buy token2 and token3 from Francesco and Shaun
echo "Buying 5token2 from Francesco..."
furyd_tx bonds buy 5token2 "100000res" "$BOND2_DID" "$FRANCESCO_DID_FULL"
echo "Buying 5token3 from Shaun..."
furyd_tx bonds buy 5token3 "100res,100rez" "$BOND3_DID" "$SHAUN_DID_FULL"
...
furyd_tx bonds buy 5token4 "2token1,2token2" "$BOND4_DID" "$MIGUEL_DID_FULL"

# ----------------------------------------------------------------------------------------- projects
# Create projects (this creates a project doc for the respective project)
SENDER_DID="$SHAUN_DID"
echo "Creating project 1/2..."
furyd_tx project create-project "$SENDER_DID" "$PROJECT1_INFO" "$PROJECT1_DID_FULL"
echo "Creating project 2/2..."
furyd_tx project create-project "$SENDER_DID" "$PROJECT2_INFO" "$PROJECT2_DID_FULL"
...
furyd_tx project update-project-status "$SENDER_DID" CREATED "$PROJECT1_DID_FULL"
echo "Updating project 2 to CREATED..."
furyd_tx project update-project-status "$SENDER_DID" CREATED "$PROJECT2_DID_FULL" --broadcast-mode block
echo "Updating project 2 to PENDING..."
furyd_tx project update-project-status "$SENDER_DID" PENDING "$PROJECT2_DID_FULL" --broadcast-mode block

# Fund project (using treasury 'send' and 'oracle-transfer')
PROJECT_2_ADDR=$(furyd_q project get-project-accounts $PROJECT2_DID | grep $PROJECT2_DID | cut -d \" -f 4)
echo "Funding project 2 [$PROJECT_2_ADDR] (using Miguel's tokens)..."
furyd_tx bank send miguel "$PROJECT_2_ADDR" 10000000000ufury --broadcast-mode block
echo "Updating project 2 to FUNDED..."
SENDER_DID="$SHAUN_DID"
furyd_tx project update-project-status "$SENDER_DID" FUNDED "$PROJECT2_DID_FULL" --broadcast-mode block
echo "Updating project 2 to STARTED..."
SENDER_DID="$SHAUN_DID"
furyd_tx project update-project-status "$SENDER_DID" STARTED "$PROJECT2_DID_FULL" --broadcast-mode block

# Adding a claim and evaluation
echo "Creating a claim in project 2..."
SENDER_DID="$SHAUN_DID"
furyd_tx project create-claim "tx_hash" "$SENDER_DID" "claim_id" "template_id" "$PROJECT2_DID_FULL" --broadcast-mode block
...
furyd_tx project create-evaluation "tx_hash" "$SENDER_DID" "claim_id" $STATUS "$PROJECT2_DID_FULL" --broadcast-mode block

# Adding agents (this creates a project account for the agent in the respective project)
echo "Adding agent to project 1..."
SENDER_DID="did:fury:48PVm1uyF6QVDSPdGRWw4T"
AGENT_DID="did:fury:RYLHkfNpbA8Losy68jt4yF"
ROLE="SA"
furyd_tx project create-agent "tx_hash" "$SENDER_DID" "$AGENT_DID" "$ROLE" "$PROJECT1_DID_FULL"
...
furyd_tx payments create-payment-template "$PAYMENT_TEMPLATE" "$CREATOR" --broadcast-mode block
...
FULL_PAYER_ADDR="$(furyd q did get-address-from-did $FRANCESCO_DID)"
PAYER_ADDR=${FULL_PAYER_ADDR##*: }
furyd_tx payments create-payment-contract "$PAYMENT_CONTRACT_ID" "$PAYMENT_TEMPLATE_ID" "$PAYER_ADDR" "$PAYMENT_RECIPIENTS" True "$DISCOUNT_ID" "$CREATOR" --broadcast-mode block
