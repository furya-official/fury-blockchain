...
furyd_tx payments create-payment-template "$PAYMENT_TEMPLATE" "$CREATOR"
...
furyd_tx payments create-payment-contract "$PAYMENT_CONTRACT_ID" "$PAYMENT_TEMPLATE_ID" "$PAYER_ADDR" "$PAYMENT_RECIPIENTS" True "$DISCOUNT_ID" "$CREATOR"

# Authorise payment contract
echo "Authorising payment contract..."
PAYER="$FRANCESCO_DID_FULL"
furyd_tx payments set-payment-contract-authorisation "$PAYMENT_CONTRACT_ID" True "$PAYER"
...
MAX_PERIODS=3
CREATOR="$SHAUN_DID_FULL"
furyd_tx payments create-subscription "$SUBSCRIPTION_ID" "$PAYMENT_CONTRACT_ID" "$MAX_PERIODS" "$PERIOD" "$CREATOR"
...
PAYER="$FRANCESCO_DID_FULL"
furyd_tx payments set-payment-contract-authorisation "$PAYMENT_CONTRACT_ID" False "$PAYER"
