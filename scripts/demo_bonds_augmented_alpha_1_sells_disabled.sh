...
echo "Created bond..."
furyd_q bonds bond "$BOND_DID"

echo "Miguel buys 400000abc..."
furyd_tx bonds buy 400000abc 500000res "$BOND_DID" "$MIGUEL_DID_FULL"
echo "Miguel's account..."
furyd_q bank balances "$MIGUEL_ADDR"

echo "Withdrawing of reserve during HATCH state not possible..."
furyd_tx bonds withdraw-reserve "$BOND_DID" 1res "$FRANCESCO_DID_FULL"

echo "Francesco buys 400000abc..."
furyd_tx bonds buy 400000abc 500000res "$BOND_DID" "$FRANCESCO_DID_FULL"
echo "Francesco's account..."
furyd_q bank balances "$FRANCESCO_ADDR"

echo "Shaun cannot buy 200001abc..."
furyd_tx bonds buy 200001abc 500000res "$BOND_DID" "$SHAUN_DID_FULL"
# echo "Shaun cannot sell anything..." ----- commented out because sells are disabled, so Shaun cannot sell in any case
# furyd_tx bonds sell 20000abc "$BOND_DID" "$SHAUN_DID_FULL"
echo "Shaun can buy 200000abc..."
furyd_tx bonds buy 200000abc 500000res "$BOND_DID" "$SHAUN_DID_FULL"
...
furyd_q bonds current-reserve "$BOND_DID"
echo "Available reserve..."
furyd_q bonds available-reserve "$BOND_DID"
echo "Withdrawing 400000res using the controller DID..."
furyd_tx bonds withdraw-reserve "$BOND_DID" 400000res "$FRANCESCO_DID_FULL"
echo "Reserve withdraw account..."
furyd_q bank balances "$RESERVE_OUT"
echo "Current reserve..."
furyd_q bonds current-reserve "$BOND_DID"
echo "Available reserve..."
furyd_q bonds available-reserve "$BOND_DID"
...
furyd_q bonds current-price "$BOND_DID"

echo "Changing public alpha 0.5->0.51..."
NEW_ALPHA="0.51"
furyd_tx bonds set-next-alpha "$NEW_ALPHA" "$BOND_DID" "$FRANCESCO_DID_FULL"
...
furyd_tx bonds withdraw-reserve "$BOND_DID" 5000res "$FRANCESCO_DID_FULL"
echo "Cannot withdraw 595001res due to insufficient funds..."
furyd_tx bonds withdraw-reserve "$BOND_DID" 595001res "$FRANCESCO_DID_FULL"
echo "Reserve withdraw account..."
furyd_q bank balances "$RESERVE_OUT"
echo "Current reserve..."
furyd_q bonds current-reserve "$BOND_DID"
...
furyd_tx bonds sell 400000abc "$BOND_DID" "$MIGUEL_DID_FULL"

echo "Francesco makes outcome payment of 50000000 [1]..."
furyd_tx bonds make-outcome-payment "$BOND_DID" "50000000" "$FRANCESCO_DID_FULL"
echo "Francesco makes outcome payment of 100000000 [2]..."
furyd_tx bonds make-outcome-payment "$BOND_DID" "100000000" "$FRANCESCO_DID_FULL"
echo "Francesco makes outcome payment of 150000000 [3]..."
furyd_tx bonds make-outcome-payment "$BOND_DID" "150000000" "$FRANCESCO_DID_FULL"
echo "Francesco's account..."
furyd_q bank balances "$FRANCESCO_ADDR"
...
furyd_tx bonds update-bond-state "SETTLE" "$BOND_DID" "$FRANCESCO_DID_FULL"
echo "Bond outcome payment reserve is now empty (moved to main reserve)..."
furyd_q bonds bond "$BOND_DID"

echo "Miguel withdraws share..."
furyd_tx bonds withdraw-share "$BOND_DID" "$MIGUEL_DID_FULL"
echo "Miguel's account..."
furyd_q bank balances "$MIGUEL_ADDR"

echo "Francesco withdraws share..."
furyd_tx bonds withdraw-share "$BOND_DID" "$FRANCESCO_DID_FULL"
echo "Francesco's account..."
furyd_q bank balances "$FRANCESCO_ADDR"

echo "Shaun withdraws share..."
furyd_tx bonds withdraw-share "$BOND_DID" "$SHAUN_DID_FULL"
echo "Shaun's account..."
furyd_q bank balances "$SHAUN_ADDR"
