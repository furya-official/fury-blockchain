...
furyd_tx() {
  ...
  NODE="https://devnet.fury.earth:443/rpc/"
...
FEE_FOR_SERVICE_PAYMENT_TEMPLATE='{
  ...
  "payment_amount": [{"denom":"ufury", "amount":"2000000"}],
  "payment_minimum": [{"denom":"ufury", "amount":"2000000"}],
  "payment_maximum": [{"denom":"ufury", "amount":"20000000"}],
  "discounts": []
}'
...
furyd_tx iid create-iid-from-legacy-did "$MIGUEL_DID_FULL"
echo "Ledgering Francesco DID..."
furyd_tx iid create-iid-from-legacy-did "$FRANCESCO_DID_FULL"
echo "Ledgering Shaun DID..."
furyd_tx iid create-iid-from-legacy-did "$SHAUN_DID_FULL"

# Create oracle fee payment template
echo "Creating oracle fee payment template..."
CREATOR="$FRANCESCO_DID_FULL"
furyd_tx payments create-payment-template "$ORACLE_FEE_PAYMENT_TEMPLATE" "$MIGUEL_DID_FULL"

# Create fee-for-service payment template
echo "Creating fee-for-service payment template..."
CREATOR="$FRANCESCO_DID_FULL"
furyd_tx payments create-payment-template "$FEE_FOR_SERVICE_PAYMENT_TEMPLATE" "$CREATOR"

# # Create project and progress status to PENDING
SENDER_DID="$SHAUN_DID"
echo "Creating project..."
furyd_tx project create-project "$SENDER_DID" "$PROJECT_INFO" "$PROJECT_DID_FULL"
# echo "Updating project to CREATED..."
# furyd_tx project update-project-status "$SENDER_DID" CREATED "$PROJECT_DID_FULL"
# echo "Updating project to PENDING..."
# furyd_tx project update-project-status "$SENDER_DID" PENDING "$PROJECT_DID_FULL"
...
# furyd_tx project update-project-doc "$SENDER_DID" "$PROJECT_INFO2" "$PROJECT_DID_FULL"

# # Fund project and progress status to FUNDED
# FULL_PROJECT_ADDR=$(furyd q project get-project-accounts $PROJECT_DID | grep "$PROJECT_DID")
# # Delete longest match of pattern ': ' from the beginning
# PROJECT_ADDR=${FULL_PROJECT_ADDR##*: }
# echo "Funding project at [$PROJECT_ADDR] with ufury from Francesco..."
# furyd_tx bank send francesco "$PROJECT_ADDR" 100000000ufury
# echo "Updating project to FUNDED..."
# SENDER_DID="$SHAUN_DID"
# furyd_tx project update-project-status "$SENDER_DID" FUNDED "$PROJECT_DID_FULL"

# # Progress project status to STARTED
# SENDER_DID="$SHAUN_DID"
# echo "Updating project to STARTED..."
# furyd_tx project update-project-status "$SENDER_DID" STARTED "$PROJECT_DID_FULL"
...
# furyd_tx project update-project-doc "$SENDER_DID" "$PROJECT_INFO" "$PROJECT_DID_FULL"

# # # Create claim and evaluation
# echo "Creating a claim in project..."
# SENDER_DID="$SHAUN_DID"
# furyd_tx project create-claim "tx_hash" "$SENDER_DID" "claim_id" "template_id" "$PROJECT_DID_FULL"
...
# furyd_tx project create-evaluation "tx_hash" "$SENDER_DID" "claim_id" $STATUS "$PROJECT_DID_FULL"

# # OracleFeePercentage:  0.1 (10%)
# # NodeFeePercentage:    0.1 (10%)

# # Fee for service:   2,000,000 ufury
# # Oracle pay:        5,000,000 ufury

# # Expected project account balances:
# # - InitiatingNodePayFees:   50,000  # 0.1 of 0.1 of oracle pay
# # - FuryFees:                      0
# # - FuryPayFees:             450,000  # 0.9 of 0.1 of oracle pay
# # - project:             93,000,000  # 100FURY - (5+2)FURY
...
# furyd_tx project update-project-status "$SENDER_DID" STOPPED "$PROJECT_DID_FULL"
# echo "Updating project to PAIDOUT..."
# furyd_tx project update-project-status "$SENDER_DID" PAIDOUT "$PROJECT_DID_FULL"
# echo "Project withdrawals query..."
# furyd_q project get-project-txs $PROJECT_DID

# # Expected withdrawals:
# # - 500,000 to fury (a.k.a Shaun) DID (did:fury:U4tSpzzv91HHqWW1YmFkHJ)
# # Expected project account balances:
...
# furyd_q bank balances "fury1xvjy68xrrtxnypwev9r8tmjys9wk0zkkspzjmq"
# echo "FuryFees"
# furyd_q bank balances "fury1ff9we62w6eyes7wscjup3p40vy4uz0sa7j0ajc"
# echo "FuryPayFees"
# furyd_q bank balances "fury1udgxtf6yd09mwnnd0ljpmeq4vnyhxdg03uvne3"
# echo "(project) did:fury:U7GK8p8rVhJMKhBVRCJJ8c"
# furyd_q bank balances "fury1rmkak6t606wczsps9ytpga3z4nre4z3nwc04p8"
# echo "(Miguel) did:fury:4XJLBfGtWSGKSz4BeRxdun"
# MIGUEL_FULL_ADDR="$(furyd q did get-address-from-did $MIGUEL_DID)"
# MIGUEL_ADDR=${MIGUEL_FULL_ADDR##*: }
# furyd_q bank balances "$MIGUEL_ADDR"
# echo "(Shaun) did:fury:U4tSpzzv91HHqWW1YmFkHJ"
# SHAUN_FULL_ADDR="$(furyd q did get-address-from-did $SHAUN_DID)"
# SHAUN_ADDR=${SHAUN_FULL_ADDR##*: }
# furyd_q bank balances "$SHAUN_ADDR"
...
furyd_tx project withdraw-funds "$MIGUEL_DID_FULL" "$DATA"
# echo "Project withdrawals query..."
# furyd_q project get-project-txs $PROJECT_DID

# # # Expected external account balances:
# # - Miguel:       1,000,004,490,000 (5,000ufury tx fee deducted)
...
# furyd_tx project withdraw-funds "$SHAUN_DID_FULL" "$DATA"
# echo "Project withdrawals query..."
# furyd_q project get-project-txs $PROJECT_DID

# # Expected withdrawals:
# # - 500,000 to fury (a.k.a Shaun) DID (did:fury:U4tSpzzv91HHqWW1YmFkHJ)
# # - 1,000,000 to shaun DID (did:fury:U4tSpzzv91HHqWW1YmFkHJ)
# # Expected project account balances:
# # - InitiatingNodePayFees:        0
# # - FuryFees:                      0
# # - FuryPayFees:                   0
