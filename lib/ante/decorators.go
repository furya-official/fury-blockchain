...
// 	// feeTx, ok := tx.(FuryFeeTx)

// 	//Check if feegranter is set. or if not furyFeeTx
...
// type FuryFeeHandlerDecorator struct {
...
// func NewFuryFeeHandlerDecorator(iidKeeper iidkeeper.Keeper, accountKeeper authante.AccountKeeper, bankKeeper bankkeeper.Keeper, defaultFeeHandler authante.DeductFeeDecorator) sdk.AnteDecorator {
// 	return FuryFeeHandlerDecorator{
// 		iidKeeper:         iidKeeper,
...
// func (dec FuryFeeHandlerDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
// 	feeTx, ok := tx.(FuryFeeTx)
...
// 	furyFeeMsgs := feeTx.GetFeePayerMsgs()

// 	furyMsgCount := len(furyFeeMsgs)
// 	if len(feeTx.GetMsgs()) != furyMsgCount && furyMsgCount == 1 {
// 		return ctx, sdkerrors.Wrapf(errors.New("only one custom fee handler msg allowed per transaction"), "expted 1 and got %d", furyMsgCount)
// 	}

// 	feePayer, err := furyFeeMsgs[0].FeePayerFromIid(ctx, dec.accountKeeper, dec.iidKeeper)
// 	if err != nil {
