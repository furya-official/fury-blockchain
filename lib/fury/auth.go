...
import (
	...
	exported "github.com/furya-official/fury-blockchain/lib/legacydid"
	"github.com/spf13/pflag"
)

var (
	expectedMinGasPrices       = "0.025" + FuryNativeToken
	approximationGasAdjustment = float64(1.5)
	fees                       = "1000000" + FuryNativeToken
...
// 	// need a custom fury AnteHandler. Below, we will discuss the differences
// 	// between the default Cosmos AnteHandler and our custom fury AnteHandler.
// 	//
...
// 	//
// 	// In general:
// 	// - Enforces messages to be of type FuryMsg, to be used with pubKeyGetter.
...
func GenerateOrBroadcastTxCLI(clientCtx client.Context, flagSet *pflag.FlagSet, furyDid exported.FuryDid, msg sdk.Msg) error {
	...
	return GenerateOrBroadcastTxWithFactory(clientCtx, txf, furyDid, msg)
}
...
func GenerateOrBroadcastTxWithFactory(clientCtx client.Context, txf tx.Factory, furyDid exported.FuryDid, msg sdk.Msg) error {
	...
	return BroadcastTx(clientCtx, txf, furyDid, msg)
}
...
func BroadcastTx(clientCtx client.Context, txf tx.Factory, furyDid exported.FuryDid, msg sdk.Msg) error {
	...
	err = Sign(txf, clientCtx, tx, true, furyDid)
	if err != nil {
		return err
	}
...
func Sign(txf tx.Factory, clientCtx client.Context, txBuilder client.TxBuilder, overwriteSig bool, furyDid exported.FuryDid) error {
	...
	privateKey.Key = append(base58.Decode(furyDid.Secret.SignKey), base58.Decode(furyDid.VerifyKey)...)
